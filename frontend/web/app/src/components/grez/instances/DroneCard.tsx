...
export const DroneCard: React.FC<DroneCardProps> = ({ drone }) => {
  ...
  return (
    <div className="w-full max-w-md mx-auto sm:max-w-full sm:px-4 md:px-6 lg:px-8 mb-6">
      <div className="rounded-2xl overflow-hidden shadow-md bg-white hover:shadow-xl transition-shadow duration-300">
        <div className="p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-2">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800">{drone.name}</h2>
            <span className={`text-xs sm:text-sm font-semibold px-3 py-1 rounded-full ${getStatusColor(drone.dstate)}`}>
              {drone.dstate.replace('_', ' ')}
            </span>
          </div>

          <p className="text-gray-600 italic mb-4 text-sm sm:text-base">{drone.description}</p>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div>
              <h3 className="text-xs sm:text-sm font-semibold text-gray-500">Modelo</h3>
              <p className="text-base sm:text-lg font-medium">{drone.model}</p>
            </div>
            <div>
              <h3 className="text-xs sm:text-sm font-semibold text-gray-500">Capacidad</h3>
              <p className="text-base sm:text-lg font-medium">{drone.capacity.split('_').join(' ')}</p>
            </div>
          </div>
